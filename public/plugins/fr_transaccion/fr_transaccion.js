	...
	jQuery(document).ready(function($) {
		...
		function converter_value_angular(tagbutton){
			detail_html = $('table#'+tagbutton+' .fr_details').html();
		...
		function extract_value_angular(tagbutton){
			...
			for(i=0; i<valores_angular.length;i++){
				...
				console.log($('table#'+tagbutton+' .fr_details').html());
				//alert(idnewtext);
				//guardamos el html del tr padre temporalmente
				tr_padre_temp = $("table#"+tagbutton+" .fr_details tr.tr_padre").html();
				//reemplazamos todos los hijos como tal
				$(".fr_details").html($("table#"+tagbutton+" .fr_details").html().replaceAll('{{'+idnewtext+'}}',new_text));
				if(new_text_value!=''){
					$(".fr_details").html($("table#"+tagbutton+" .fr_details").html().replaceAll('{{'+array_fr_select[0]+'-value}}',new_text_value));
				}
				//retornamos nuevamente los valores angular al tr padre
				$("table#"+tagbutton+" tr.tr_padre").html(tr_padre_temp);
			}
		...
		$(document).on('click','.fr_button_add',function(){
			tagbutton = $(this).attr('tagbutton');
			/*si la validacion no devolvio nimgun error entra*/
			if(fr_validate('button-click')==0){
				//clonamos el tr padre al hijo
				$("table#"+tagbutton+" .fr_details").find('.tr_padre').clone().removeClass('tr_padre').addClass('tr_hijo').appendTo('.fr_details');
